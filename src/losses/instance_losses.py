...
class InstanceMetricLoss(torch.nn.Module):
    def __init__(
        self,
        loss_fn_name: str = "MultiSimilarityLoss",
        miner_name: str| None = "MultiSimilarityMiner",
    ):
        super().__init__()
        # store names for external checks
        self.loss_fn_name = loss_fn_name
        self.miner_name = miner_name

        # create base loss and miner instances
        base_loss = self._get_loss(loss_fn_name)
        base_miner = self._get_miner(miner_name)

        # Wrap with distributed wrappers only if process group initialized
        if dist.is_available() and dist.is_initialized():
            self.loss_fn = pml_dist.DistributedLossWrapper(base_loss, efficient=False)
            self.miner = pml_dist.DistributedMinerWrapper(base_miner, efficient=False) if base_miner is not None else None
        else:
            # single-process / non-initialized distributed: don't wrap
            self.loss_fn = base_loss
            self.miner = base_miner

        print_rank_0(f"[{self.__class__.__name__}] Using loss function: {loss_fn_name}")
        if self.miner is not None:
            print_rank_0(f"[{self.__class__.__name__}] Using miner: {miner_name}")
    ...
    def forward(self, embeddings: torch.Tensor, labels: torch.Tensor):
        ...
        if self.miner is not None:
            ...
            nb_samples = embeddings.shape[0] * world_size   # number of samples in the batch
    ...
    def _get_loss(self, loss_name):
        ...
        if "InstanceLoss" in loss_name:
            return losses.InstanceLoss(
                gamma=64,
            )
        
        if "RankedListLoss" in loss_name:
            return losses.RankedListLoss(
                margin=0.4,
                Tn=1.0,
                imbalance=0.5,
                alpha= None,
        ...
        if "SoftTripleLoss" in loss_name:
            return losses.SoftTripleLoss(
                num_classes=1000,
                embedding_size=512,
